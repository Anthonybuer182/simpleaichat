...
You can also pass the API key by storing it in an `.env` file with a `API_KEY` field in the working directory (recommended), or by setting the environment variable of `API_KEY` directly to the API key.
